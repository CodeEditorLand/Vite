...
import { commonFsUtils, type FsUtils } from "../fsUtils";
import {
...
export function resolvePlugin(resolveOptions: InternalResolveOptions): Plugin {
	...
	const rootInRoot =
		tryStatSync(path.join(root, root))?.isDirectory() ?? false;
	...
	return {
		...
		async resolveId(id, importer, resolveOpts) {
			...
			if (resolvedImports) {
				...
				if (
					resolveOpts.custom?.["vite:import-glob"]
						?.isSubImportsPattern
				) {
			...
			if (importer) {
				if (
					...
					const moduleLang =
						this.getModuleInfo(importer)?.meta?.vite?.lang;
			...
			if (
				...
				if ((res = tryFsResolve(fsPath, options))) {
					...
					debug?.(
						`[relative] ${colors.cyan(id)} -> ${colors.dim(res)}`,
					);
			...
			if (isWindows && id[0] === "/") {
				...
				if ((res = tryFsResolve(fsPath, options))) {
					debug?.(
						`[drive-relative] ${colors.cyan(id)} -> ${colors.dim(res)}`,
					);
					return ensureVersionQuery(res, id, options, depsOptimizer);
				}
			}
...
function tryResolveBrowserMapping(
	...
	if (pkg && isObject(pkg.data.browser)) {
		const mapId = isFilePath
			? "./" + slash(path.relative(pkg.dir, id))
			: id;
		const browserMappedPath = mapWithBrowserField(mapId, pkg.data.browser);
		if (browserMappedPath) {
			if (
				(res = bareImportRE.test(browserMappedPath)
					? tryNodeResolve(browserMappedPath, importer, options, true)
							?.id
					: tryFsResolve(
				...
				debug?.(
					`[browser mapped] ${colors.cyan(id)} -> ${colors.dim(res)}`,
				);
