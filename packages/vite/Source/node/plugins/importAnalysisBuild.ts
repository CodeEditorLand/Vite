...
function preload(
	...
	if (__VITE_IS_MODERN__ && deps && deps.length > 0) {
		...
		const cspNonce =
			cspNonceMeta?.nonce || cspNonceMeta?.getAttribute("nonce");
	...
	return {
			...
			if (insertPreload) {
				...
				while ((match = dynamicImportTreeshakenRE.exec(source))) {
					...
					if (match[3]) {
						...
						dynamicImports[
							dynamicImportTreeshakenRE.lastIndex -
								match[4]?.length -
								1
						] = {
		...
		generateBundle({ format }, bundle) {
			...
			for (const file in bundle) {
				...
				if (
					chunk.type === "chunk" &&
					chunk.code.indexOf(preloadMarker) > -1
				) {
					const code = chunk.code;
					let imports!: ImportSpecifier[];
					try {
					...
					const addFileDep = (
						...
						const index = fileDeps.findIndex(
							(dep) => dep.url === url,
						);
						if (index === -1) {
								...
								const addDeps = (filename: string) => {
									...
									if (chunk) {
										...
										if (chunk.type === "chunk") {
											...
											chunk.viteMetadata!.importedCss.forEach(
												(file) => {
													deps.add(file);
												},
											);
										...
										const removedPureCssFiles =
											removedPureCssFilesCache.get(
												config,
											)!;
										const chunk =
											removedPureCssFiles.get(filename);
										if (chunk) {
											if (
