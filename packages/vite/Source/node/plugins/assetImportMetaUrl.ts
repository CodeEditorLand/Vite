...
import { tryFsResolve, type InternalResolveOptions } from "./resolve";
			...
			if (
				...
				while ((match = assetImportMetaUrlRE.exec(cleanString))) {
					const [[startIndex, endIndex], [urlStart, urlEnd]] =
						match.indices!;
					if (hasViteIgnoreRE.test(code.slice(startIndex, urlStart)))
						continue;
					...
					if (rawUrl[0] === "`" && rawUrl.includes("${")) {
						const queryDelimiterIndex =
							getQueryDelimiterIndex(rawUrl);
					...
					if (file) {
						try {
							if (
								publicDir &&
								isParentDirectory(publicDir, file)
							) {
								const publicPath =
									"/" + path.posix.relative(publicDir, file);
								builtUrl = await fileToUrl(
									publicPath,
									config,
									this,
								);
