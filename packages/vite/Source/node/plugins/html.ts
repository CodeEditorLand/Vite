	...
	return {
			...
			if (id.endsWith(".html")) {
				...
				await traverseHtml(html, id, (node) => {
					...
					if (node.nodeName === "script") {
						...
						const isPublicFile = !!(
							url && checkPublicFile(url, config)
						);
						if (isPublicFile) {
								...
								if (attrKey === "srcset") {
									assetUrlsPromises.push(
										(async () => {
											...
											if (
												processedEncodedUrl !== p.value
											) {
												overwriteAttrValue(
													s,
													getAttrSourceCodeLocation(
														node,
														attrKey,
													),
													processedEncodedUrl,
									...
									if (checkPublicFile(url, config)) {
										...
										if (
											...
											styleUrls.push({
												url,
												start: nodeStartWithLeadingWhitespace(
													node,
												),
												end: node.sourceCodeLocation!
													.endOffset,
											});
											js += importExpression;
										} else {
												...
												node.attrs.some(
													(p) =>
														...
														parseRelAttr(
															...
															noInlineLinkRels.has(
																v,
															),
														),
												);
											const shouldInline = isNoInlineLink
												? false
												: undefined;
											assetUrlsPromises.push(
												(async () => {
		...
		async generateBundle(options, bundle) {
			...
			for (const [normalizedId, html] of processedHtml) {
				...
				const isAsync = isAsyncScriptMap
					.get(config)!
					.get(normalizedId)!;
				...
				if (chunk) {
					...
					if (canInlineEntry) {
						...
						if (modulePreload !== false) {
							...
							const importsFileNames = imports.map(
					...
					assetTags.push(
						...getCssTagsForChunk(chunk, toOutputAssetFilePath),
					);
				...
				result = result.replace(
					...
					(_, fileHash, postfix = "") => {
						...
						return (
							encodeURIPath(toOutputAssetFilePath(file)) + postfix
						);
					},
				);
			...
			if (
				...
				(nodeName === "link" &&
					attrs.some(
						(attr) =>
							attr.name === "rel" &&
							parseRelAttr(attr.value).some((a) =>
								processRelType.has(a),
							),
					))
			) {
	...
	if (prepend) {
		...
		if (headInjectRE.test(html)) {
			...
			return html.replace(
				headInjectRE,
				(match, p1) =>
					`${serializeTags(tags, incrementIndent(p1))}${match}`,
			);
		}
