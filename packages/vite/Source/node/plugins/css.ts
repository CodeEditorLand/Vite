...
export function cssPlugin(config: ResolvedConfig): Plugin {
	...
	return {
		...
		buildStart() {
			...
			removedPureCssFilesCache.set(
				config,
				new Map<string, RenderedChunk>(),
			);
			...
			if (urlRE.test(id)) {
				...
				if (isBuild) {
					...
					return (
						...
						`export default "__VITE_CSS_URL__${Buffer.from(
							id,
						).toString("hex")}__"`
					);
				}
	...
	return {
		...
		async transform(css, id, options) {
			...
			return {
				...
				// it in renderChunk()
				moduleSideEffects:
					modulesCode || inlined ? false : "no-treeshake",
			};
		},
		...
		async renderChunk(code, chunk, opts) {
			...
			if (urlEmitTasks.length > 0) {
				...
				for (const {
					...
					generatedAssets
						.get(config)!
						.set(referenceId, { originalFileName });
					...
					const replacementString =
						typeof replacement === "string"
							? JSON.stringify(encodeURIPath(replacement)).slice(
									1,
									-1,
								)
							: `"+${replacement.runtime}+"`;
			...
			if (chunkCSS) {
				...
				if (config.build.cssCodeSplit) {
					if (opts.format === "es" || opts.format === "cjs") {
						...
						const cssFullAssetName = ensureFileExt(
							chunk.name,
							".css",
						);
						...
						if (wrapIdx >= 0) {
							const executeFnStart = code.indexOf(
								"execute:",
								wrapIdx,
							);
							injectionPoint =
								code.indexOf("{", executeFnStart) + 1;
						} else {
							const insertMark = "'use strict';";
							injectionPoint =
								code.indexOf(insertMark) + insertMark.length;
						}
						s ||= new MagicString(code);
		...
		async generateBundle(opts, bundle) {
			...
			if (pureCssChunks.size) {
				...
				const pureCssChunkNames = [...pureCssChunks]
					.map(
						(pureCssChunk) =>
							prelimaryNameToChunkMap[pureCssChunk.fileName],
					)
					.filter(Boolean);
				...
				pureCssChunkNames.forEach((fileName) => {
					removedPureCssFiles.set(
						fileName,
						bundle[fileName] as RenderedChunk,
					);
	...
	return {
		...
		async transform(_, id, options) {
			...
			if (thisModule) {
				...
				const pluginImports = (
					this as unknown as TransformPluginContext
				)._addedImports;
				if (pluginImports) {
		...
		postcssPlugins.unshift(
			(await importPostcssImport()).default({
				...
				async load(id) {
					...
					const lang = CSS_LANGS_RE.exec(id)?.[1] as
						| CssLang
						| undefined;
					if (isPreProcessor(lang)) {
		...
		postcssPlugins.unshift(
			(await importPostcssModules()).default({
				...
				getJSON(
					...
					if (
						modulesOptions &&
						typeof modulesOptions.getJSON === "function"
					) {
						modulesOptions.getJSON(
							cssFileName,
							_modules,
							outputFileName,
						);
	...
	try {
		...
		for (const message of postcssResult.messages) {
			if (message.type === "dependency") {
				...
				const pattern =
					glob.escapePath(
						normalizePath(path.resolve(path.dirname(id), dir)),
					) +
					`/` +
					globPattern;
...
export async function formatPostcssSourceMap(
	...
	const sources = rawMap.sources.map((source) => {
		...
		if (
			cleanSource[0] === "<" &&
			cleanSource[cleanSource.length - 1] === ">"
		) {
		...
		() =>
			async (
				...
				const _internalImporter: Sass.LegacyAsyncImporter = (
					...
					internalImporter(url, importer, options.filename).then(
						(data) => done?.(data),
					);
				};
				...
				const internalImporter: Sass.Importer<"async"> = {
					async canonicalize(url, context) {
						const importer = context.containingUrl
							...
							: options.filename;
						const resolved = await internalCanonicalize(
							url,
							importer,
						);
	...
	const worker: Awaited<ReturnType<typeof makeModernScssWorker>> = {
		async run(sassPath, data, options) {
			...
			const sass: typeof Sass = (
				await import(pathToFileURL(sassPath).href)
			).default;
			...
			const internalImporter: Sass.Importer<"async"> = {
				...
				async load(canonicalUrl) {
					...
					const contents =
						result.contents ??
						(await fsp.readFile(result.file, "utf-8"));
					return { contents, syntax };
				},
			};
			sassOptions.importers = [
	...
	return {
		...
		async process(source, root, options, resolvers) {
			...
			try {
				...
				const deps = result.stats.includedFiles.map((f) =>
					cleanScssBugUrl(f),
				);
...
async function rebaseUrls(
	...
	const rebaseFn = async (url: string) => {
		...
		const absolute =
			(await resolver(url, file)) || path.resolve(fileDir, url);
