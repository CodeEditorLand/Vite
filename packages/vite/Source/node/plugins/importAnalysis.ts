...
import {
	...
	type ParseError as EsModuleLexerParseError,
...
import { getDepOptimizationConfig, type ResolvedConfig } from "../config";
import {
	CLIENT_DIR,
	CLIENT_PUBLIC_PATH,
	DEP_VERSION_RE,
		...
		async transform(source, importer, options) {
			...
			const normalizeUrl = async (
				...
				if (!resolved || resolved.meta?.["vite:alias"]?.noResolved) {
					...
					importerModule.isSelfAccepting = false;
					moduleGraph._hasResolveFailedErrorModules.add(
						importerModule,
					);
					return this.error(
			...
			const orderedImportedUrls = new Array<string | undefined>(
				imports.length,
			);
				...
				imports.map(async (importSpecifier, index) => {
					...
					if (rawUrl === "import.meta") {
						...
						if (prop === ".hot") {
							hasHMR = true;
							const endHot =
								end + 4 + (source[end + 4] === "?" ? 1 : 0);
							if (
								source.slice(endHot, endHot + 7) === ".accept"
							) {
								...
								if (
									...
									const importAcceptedExports =
										(orderedAcceptedExports[index] =
											new Set<string>());
									lexAcceptedHmrExports(
										source,
					...
					if (specifier !== undefined) {
						...
						if (url !== specifier) {
							...
							if (
								...
								const needsInterop =
									await optimizedDepNeedsInterop(
										depsOptimizer.metadata,
										file,
										config,
										ssr,
						...
						const isLocalImport =
							!isExternalUrl(hmrUrl) && !isDataUrl(hmrUrl);
						if (isLocalImport) {
						...
						!importer.startsWith(withTrailingSlash(clientDir))
					) {
			...
			const isClassicWorker =
				importer.includes(WORKER_FILE_ID) &&
				importer.includes("type=classic");
