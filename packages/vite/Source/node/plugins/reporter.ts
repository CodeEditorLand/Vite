		...
		renderChunk(code, chunk, options) {
			if (!options.inlineDynamicImports) {
				for (const id of chunk.moduleIds) {
					...
					if (
						module.importers.length &&
						module.dynamicImporters.length
					) {
								...
								? colors.dim(assetsDir) +
									group.color(
										entry.name
											...
											.padEnd(
												longest + 2 - assetsDir.length,
											),
									)
								: group.color(entry.name.padEnd(longest + 2));
						log += colors.bold(
							sizeColor(
								displaySize(entry.size).padStart(sizePad),
							),
						);
