...
import { printServerUrls, type Logger } from "../logger";
import { getDepsOptimizer, initDepsOptimizer } from "../optimizer";
import { ERR_OUTDATED_OPTIMIZED_DEP } from "../plugins/optimizedDeps";
import { initPublicFiles } from "../publicDir";
import { bindCLIShortcuts, type BindCLIShortcutsOptions } from "../shortcuts";
...
import { ModuleGraph, type ModuleNode } from "./moduleGraph";
...
import { createWebSocketServer, type WebSocketServer } from "./ws";
...
export async function _createServer(
	...
	if (typeof config.server.hmr === "object" && config.server.hmr.channels) {
		config.server.hmr.channels.forEach((channel) =>
			hot.addChannel(channel),
		);
	}
		...
		openBrowser() {
			...
			if (url) {
				...
				if (server.config.server.preTransformRequests) {
					setTimeout(() => {
						const getMethod = path.startsWith("https:")
							? httpsGet
							: httpGet;
			...
			if (file.startsWith(publicDir)) {
				...
				if (!isUnlink) {
					const moduleWithSamePath =
						await moduleGraph.getModuleByUrl(path);
...
async function restartServer(server: ViteDevServer) {
	...
	// This allows us to keep the same server instance for the user.
	{
		...
		try {
			...
			server.config.logger.error("server restart failed", {
				timestamp: true,
			});
			return;
		}
