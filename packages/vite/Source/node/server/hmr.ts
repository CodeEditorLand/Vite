...
import { restartServerWithUrls, type HttpServer } from ".";
...
function propagateUpdate(
	...
	for (const importer of node.importers) {
		...
		if (node.id && node.acceptedHmrExports && importer.importedBindings) {
			const importedBindingsFromNode = importer.importedBindings.get(
				node.id,
			);
			if (
	...
	const broadcaster: HMRBroadcaster = {
		...
		addChannel(channel) {
			if (channels.some((c) => c.name === channel.name)) {
				throw new Error(
					`HMR channel "${channel.name}" is already defined.`,
				);
			}
