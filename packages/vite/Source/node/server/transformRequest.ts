	...
	if (pending) {
		return server.moduleGraph
			...
			.then((module) => {
				if (
					!module ||
					pending.timestamp > module.lastInvalidationTimestamp
				) {
...
async function doTransform(
	...
	if (module) {
		...
		await server.moduleGraph._ensureEntryFromUrl(
			url,
			ssr,
			undefined,
			resolved,
...
async function loadAndTransform(
	...
	if (code == null) {
		...
		const msg = isPublicFile
			? `This file is in ${publicDirName} and will be copied as-is during ` +
				...
				`via HTML tags.`
			: `Does the file exist?`;
		const importerMod: ModuleNode | undefined =
	...
	mod ??= await moduleGraph._ensureEntryFromUrl(
		url,
		ssr,
		undefined,
		resolved,
