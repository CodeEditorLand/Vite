...
export function cachedTransformMiddleware(
	...
	return function viteCachedTransformMiddleware(req, res, next) {
		...
		if (ifNoneMatch) {
			const moduleByEtag =
				server.moduleGraph.getModuleByEtag(ifNoneMatch);
			if (moduleByEtag?.transformResult?.etag === ifNoneMatch) {
				...
				if (!maybeMixedEtag) {
					debugCache?.(
						`[304] ${prettifyUrl(req.url!, server.config.root)}`,
					);
					res.statusCode = 304;
					return res.end();
				}
			...
			if (isSourceMap) {
				...
				if (depsOptimizer?.isOptimizedDepUrl(url)) {
					...
					const sourcemapPath = url.startsWith(FS_PREFIX)
						? fsPathFromId(url)
						: normalizePath(
								path.resolve(server.config.root, url.slice(1)),
							);
					try {
			...
			if (
				...
				if (isCSSRequest(url)) {
					...
					if (
						...
						debugCache?.(
							`[304] ${prettifyUrl(url, server.config.root)}`,
						);
						res.statusCode = 304;
						return res.end();
					}
				...
				if (result) {
					...
					const isDep =
						...
						depsOptimizer?.isOptimizedDepUrl(url);
