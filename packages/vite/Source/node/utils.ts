...
const limitFlattenIdLength = (
	...
	return (
		id.slice(0, limit - (FLATTEN_ID_HASH_LENGTH + 1)) + "_" + getHash(id)
	);
};
...
function optimizeSafeRealPathSync() {
	...
	try {
		...
		if (
			error.message.includes("EISDIR: illegal operation on a directory")
		) {
			safeRealpathSync = fs.realpathSync;
			return;
		}
...
function joinSrcset(ret: ImageCandidate[]) {
	return ret
		.map(
			({ url, descriptor }) => url + (descriptor ? ` ${descriptor}` : ""),
		)
		.join(", ");
}
...
export function combineSourcemaps(
	...
	sourcemapList = sourcemapList.map((sourcemap) => {
		...
		if (sourcemap.sourceRoot) {
			newSourcemaps.sourceRoot = escapeToLinuxLikePath(
