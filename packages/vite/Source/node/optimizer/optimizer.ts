...
async function createDepsOptimizer(
	...
	let metadata =
		cachedMetadata ||
		initDepsOptimizerMetadata(config, ssr, sessionTimestamp);
			...
			depsOptimizer.scanProcessing = new Promise((resolve) => {
				...
				(async () => {
					try {
						...
						const manuallyIncluded = Object.keys(
							manuallyIncludedDepsInfo,
						);
						discoveredDepsWhileScanning.push(
							...Object.keys(metadata.discovered).filter(
								(dep) =>
									!deps[dep] &&
									!manuallyIncluded.includes(dep),
							),
						);
...
function findInteropMismatches(
	...
	for (const dep in discovered) {
		...
		if (depInfo.needsInterop !== discoveredDepInfo.needsInterop) {
			...
			debug?.(
				colors.cyan(`✨ needsInterop mismatch detected for ${dep}`),
			);
		}
	}
