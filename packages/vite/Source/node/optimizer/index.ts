	...
	if (!force) {
		...
		if (cachedMetadata) {
			if (cachedMetadata.lockfileHash !== getLockfileHash(config, ssr)) {
				...
				cachedMetadata.configHash !== getConfigHash(config, ssr)
			) {
				config.logger.info(
		...
		commit: async () => {
			...
			if (isWindows) {
				if (depsCacheDirPresent) {
					debug?.(
						colors.green(
							`renaming ${depsCacheDir} to ${temporaryPath}`,
						),
					);
				...
				for (const chunk of result.output) {
					if (chunk.type === "chunk") {
						if (chunk.isEntry) {
							...
							const id = chunk.fileName.replace(
								jsExtensionRE,
								"",
							);
							const file = getOptimizedDepPath(
								id,
								resolvedConfig,
								ssr,
							);
...
async function prepareRolldownOptimizerRun(
	...
	build?: () => Promise<rolldown.RollupOutput>;
	...
	const plugins = await asyncFlatten(
		Array.isArray(pluginsFromConfig)
			? pluginsFromConfig
			: [pluginsFromConfig],
	);
	if (external.length) {
...
export function createIsOptimizedDepUrl(
	...
	const depsCacheDirRelative = normalizePath(
		path.relative(root, depsCacheDir),
	);
...
export async function extractExportsData(
	...
	try {
		...
		const loader =
			rollupOptions.moduleTypes?.[path.extname(filePath)] || "jsx";
		debug?.(
	...
	const content = JSON.stringify(
		{
			...
			optimizeDeps: {
				...
				esbuildOptions: {
					...
					plugins: optimizeDeps?.esbuildOptions?.plugins?.map(
						(p) => p.name,
					),
				},
			},
...
export async function cleanupDepsCacheStaleDirs(
	...
	try {
		...
		if (fs.existsSync(cacheDir)) {
			const dirents = await fsp.readdir(cacheDir, {
				withFileTypes: true,
			});
