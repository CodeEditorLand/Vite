...
export class ViteRuntime {
	...
	constructor(
		...
		this.moduleCache =
			options.moduleCache ?? new ModuleCacheMap(options.root);
	...
	protected async directRequest(
		...
		const meta: ViteRuntimeImportMeta = {
			...
			glob() {
				throw new Error(
					'[vite-runtime] "import.meta.glob" is not supported.',
				);
			},
		};
		...
		if (this.hmrClient) {
			Object.defineProperty(meta, "hot", {
				...
				get: () => {
					if (!this.hmrClient) {
						throw new Error(
							`[vite-runtime] HMR client was destroyed.`,
						);
					}
					this.debug?.(
						"[vite-runtime] creating hmr context for",
						moduleId,
					);
