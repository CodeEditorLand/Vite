...
function viteLegacyPlugin(options: Options = {}): Plugin[] {
		...
		async generateBundle(opts, bundle) {
			...
			if (!isLegacyBundle(bundle, opts)) {
				...
				if (genLegacy && polyfillChunk) {
					polyfillChunk.code =
						modernChunkLegacyGuard + polyfillChunk.code;
				}
				return;
			}
	...
	const legacyPostPlugin: Plugin = {
		...
		configResolved(_config) {
			...
			isDebug &&
				console.log(
					`[@vitejs/plugin-legacy] modernTargets:`,
					modernTargets,
				);
		...
		async renderChunk(raw, chunk, opts, { chunks }) {
			...
			if (chunkFileNameToPolyfills == null) {
				...
				outputToChunkFileNameToPolyfills.set(
					opts,
					chunkFileNameToPolyfills,
				);
			}
			const polyfillsDiscovered = chunkFileNameToPolyfills.get(
				chunk.fileName,
			);
			...
			const needPolyfills =
				options.polyfills !== false &&
				!Array.isArray(options.polyfills);
